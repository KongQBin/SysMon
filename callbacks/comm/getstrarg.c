...
int getRegsStrArg(struct pidinfo *info, int arg, char **str, size_t *len)
{
    *str = NULL;
    int ret = 0, run = 1, j = 0;
...
int getRealPath(struct pidinfo *info, char **str, size_t *len)
{
    ...
    if(*str[0] == '.')
    {
        ...
        if(!getCwd(info,&cwd,&cwdlen))
        {
            ...
            char *tmp = realloc(cwd,cwdlen+*len+2);
            if(tmp)
            {
                cwd = tmp;
                ...
                *str = cwd;
                ...
                else
                {
                     ...
                     * 转换是因为转换后的/home/kongbin/sysmon更加易于人类阅读
                     */
